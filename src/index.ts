export const type = "zeroclue_opencode";
export const label = "ZeroClue OpenCode";
...
export const agentConfigurationDoc = `# zeroclue_opencode agent configuration

Adapter: zeroclue_opencode
...
- Paperclip requires an explicit \`model\` value for \`zeroclue_opencode\` agents.
...
// ---------------------------------------------------------------------------
// External-plugin entry point (plugin-loader convention)
// Mirrors the upstream registry's opencode_local module assembly, with the
// session-management policy inlined (parity with opencode_local's entry:
// legacy-sessioned defaults) and no dependency on the builtin package.
// ---------------------------------------------------------------------------

import { execute } from "./server/execute.js";
import { testEnvironment } from "./server/test.js";
import { sessionCodec } from "./server/index.js";
import { listOpenCodeSkills, syncOpenCodeSkills } from "./server/skills.js";
import { listOpenCodeModels, refreshOpenCodeModels } from "./server/models.js";
import { getOpenCodeStats } from "./server/stats.js";
import type { ServerAdapterModule } from "@paperclipai/adapter-utils";

export function createServerAdapter(): ServerAdapterModule {
  return {
    type: "zeroclue_opencode",
    execute,
    testEnvironment,
