...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // ZeroClue fix (belt-and-braces with --dir): the spawned opencode process
  ...
  try {
    ...
    const buildArgs = (resumeSessionId: string | null) => {
      ...
      // ZeroClue fix: bind the session directory explicitly. Without this,
      // opencode derives session.directory from inherited env.PWD (the
      // paperclip server's launch dir), so every agent session executed
      // inside /home/arminm/projects/paperclip regardless of spawn cwd.
